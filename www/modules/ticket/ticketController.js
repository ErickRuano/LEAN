app.controller('ticketController', function($scope, $rootScope, $state, $timeout, $http, api, localStorage, dialog, $mdDialog, $mdToast, $interval, helpdesk) {

    $scope.comment = "";
    $scope.createComment = function() {
        helpdesk.get('/api/helpdesk/addResponse', {
            iduser: $rootScope.user.id,
            redmadre: 10,
            comment: $scope.comment,
            idTicket: $rootScope.inTicket.id,
            source: 3
        })
            .success(function(data) {
                helpdesk.get('/api/helpdesk/ticketDetailExternal', {
                    idTicket: $rootScope.inTicket.id
                }).success(function(data) {
                    console.info(data);
        ...
        helpdesk.get('/api/helpdesk/ticketDetailExternal', {idTicket : $state.params.id}).success(function(data){
            console.info(data);
            $rootScope.ticketDetail = data;
        });
    ...
    api.agent.getOne({
        ...
        success: function(data) {
            ...
            if (data.profile == null) {
                data.profile = 'No definido';
            } else {
                api.profile.getOne({
                    params: name,
                    id: data.profile,
                    success: function(agentProfile) {
                        data.profile = agentProfile.name;
                    },
